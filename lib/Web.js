...
app.get('/users/:user/associations', function (req, res) {
    pushAssociations.getForUser(req.params.user, function (err, items) {
        if (!err) {
            ...
            res.status(503).send();
            errlog("API '/users/:user/associations'", err);
        }
    });
});
...
function sendNotifications(notifs) {
    log("API got notifications to send", notifs);
    ...
    notifs.forEach(function (notif) {
        ...
        var fetchUsers = users ? pushAssociations.getForUsers : pushAssociations.getAll,
            callback = function (err, pushAssociations) {
                if (err) {
                    errlog("Sending failed when getting users", err);
                }
                ...
                push.send(pushAssociations, androidPayload, iosPayload, config.get("dryRun"));
            },
...
exports.start = function () {
    app.listen(config.get('webPort'), config.get("webHost"));
    log('Listening on port ' + config.get('webPort') + ' and host ' + config.get("webHost"));

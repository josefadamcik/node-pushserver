...
var onTransmissionError = function (errorCode, notification, recipient) {
    console.error(new Date().toString() + ": " +'Error while pushing to APN: ' + errorCode);

    // Invalid token => remove device
    if (errorCode === 8) {
        var token = recipient.token.toString('hex').toUpperCase();

        console.log(new Date().toString() + ": " +'Invalid token: removing device ' + token );
...
var onFeedback = function (deviceInfos) {
    console.log(new Date().toString() + ": " +'Feedback service, number of devices to remove: ' + deviceInfos.length);
    ...
    if(deviceInfos.length > 0){
        pushAssociations.removeDevices(deviceInfos.map(function (deviceInfo) {
            try {
                ...
                console.error(new Date().toString() + " : " + "Can not remove the device " + deviceInfo);
                throw err;
            }
        }));
    }
